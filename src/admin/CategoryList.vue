      ...
      <el-table-column label="操作" width="500">
        <template v-slot="scope">
          ...
          <!-- <el-popconfirm icon="el-icon-delete" iconColor="red" title="确定删除吗？" @confirm="deleteTypeById(scope.row.id)"> -->
            <el-button slot="reference" icon="el-icon-delete" size="mini" type="danger" @click="deleteTypeById(scope.row.id)">删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        categoryName: [{required: true, message: '名称', trigger: 'blur'}],
      }
    }
  },
  methods:{
    ...
    getTypeList() {
      const _this = this
      axios.get('/categorys/list?currentPage=' + this.queryInfo.currentPage+"&pageSize=" + this.queryInfo.pageSize).then((res) => {
  ...
  deleteTypeById(id) {
     this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
       ...
       type: 'warning'
     	...
     	// ---------------------------------------------
     const _this = this
     axios.get('/categorys/delete/' + id).then((res) => {
     		  _this.getTypeList()
     })
     	// ---------------------------------------------
    ...
    saveType(){
      const _this = this
      axios.post('/categorys/create',this.addForm).then((res) => {
        _this.$alert('操作成功', '提示', {
          confirmButtonText: '确定',
          callback: action => {
    ...
    editType(){
      const _this = this
      axios.post('/categorys/update', this.editForm).then(res => {
        _this.$alert('操作成功', '提示', {
          confirmButtonText: '确定',
          callback: action => {
