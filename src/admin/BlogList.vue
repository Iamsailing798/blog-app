<template>
  <div class="bloglist-contain">
    ...
    <el-table :data="blogList">
	  ...
	  <el-table-column label="摘要" prop="summary" width="750"></el-table-column>
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          ...
          <!-- <el-popconfirm icon="el-icon-delete" iconColor="red" title="确定删除吗？" @confirm="deleteBlogById(scope.row.id)"> -->
            <el-button slot="reference" icon="el-icon-delete" size="mini" type="danger" @click="deleteBlogById(scope.row.id)">删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      const _this = this
      axios.get('/blogList?currentPage=' + this.queryInfo.pageNum+"&pageSize=" + this.queryInfo.pageSize
	  ,{
        headers: {
    ...
    deleteBlogById(blogId) {
     this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
       ...
       type: 'warning'
     	...
     	// ---------------------------------------------
     	const _this = this
     	axios.get('/blog/delete/' + blogId).then((res) => {
			_this.getData()
     	}),
     	// ---------------------------------------------
