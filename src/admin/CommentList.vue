...
export default {
  ...
  methods:{
    ...
    getCommentList() {
      const _this = this
      axios.get('/comments/commentList?currentPage=' + this.queryInfo.pageNum+"&pageSize=" + this.queryInfo.pageSize,{
        headers: {
    ...
    deleteCommentById(id){
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        type: 'warning'
      	...
      	// ---------------------------------------------
      const _this = this
      axios.get('/comments/delete/'+id).then((res) => {
    ...
    editComment(){
      const _this = this
      axios.post('/comments/update',this.editForm).then((res) => {
        _this.$alert('操作成功', '提示', {
          confirmButtonText: '确定',
          callback: action => {
            _this.getCommentList()
    ...
    search(){
      const _this = this
      axios.get('/comment/detail?blogId='+this.queryInfo.blogId+"&currentPage=" + this.queryInfo.pageNum+"&pageSize=" + this.queryInfo.pageSize,{
        headers: {
          "Authorization": localStorage.getItem("token")
        }
      }).then((res) => {
